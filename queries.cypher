...
CALL apoc.export.csv.query("MATCH (n) RETURN artifact.coordinates AS artifact, size((n)<-[:DEPENDS_ON]-()) as direct_usages ORDER BY direct_usages DESC","%PATH%/direct_usages.csv", {});

CALL apoc.export.csv.query("MATCH (n) RETURN artifact.coordinates AS artifact, size((n)-[:DEPENDS_ON]->()) as direct_dependencies ORDER BY direct_dependencies DESC","%PATH%/direct_dependencies.csv", {});
